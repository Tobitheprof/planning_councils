...
class PlanningSpider(scrapy.Spider):
    name = "adur_council"
    allowed_domains = ["planning.adur-worthing.gov.uk"]

    def start_requests(self):
        url = "https://planning.adur-worthing.gov.uk/online-applications/search.do?action=weeklyList"
    ...
    def parse_details_page(self, response):
        ...
        application_item['documents'] = "https://planning.adur-worthing.gov.uk" + str(response.css('p.associateddocument a::attr(href)').get())
        application_item['cases'] = "https://planning.adur-worthing.gov.uk" + str(response.css("p.associatedcase a::attr(href)").get())
        application_item['property'] = "https://planning.adur-worthing.gov.uk" + str(response.css("p.associatedproperty a::attr(href)").get())
