...
class PlanningSpider(scrapy.Spider):
    name = "basildon_borough"
    allowed_domains = ["planning.basildon.gov.uk"]

    def start_requests(self):
        url = "https://planning.basildon.gov.uk/online-applications/search.do?action=weeklyList"
    ...
    def parse_details_page(self, response):
        ...
        application_item['documents'] = "https://planning.basildon.gov.uk/" + str(response.css('p.associateddocument a::attr(href)').get())
        application_item['cases'] = "https://planning.basildon.gov.uk/" + str(response.css("p.associatedcase a::attr(href)").get())
        application_item['property'] = "https://planning.basildon.gov.uk/" + str(response.css("p.associatedproperty a::attr(href)").get())
